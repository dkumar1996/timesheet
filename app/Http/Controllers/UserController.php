...
class UserController extends Controller {
  ...
  public function store()
  {
    ...
    if($validator->passes())
    {
      $credentials=['password'=>request()->input('pin'),'email'=>request()->input('email')];
      ...
      if(Auth::user()->validate($credentials))
      {
        ...
        {
          $test=DB::table('user_attendance')->where('userid',$userid)->where('created_date',$currentdate)->where('site_id',request()->input('site_id'))->where('flag_login',1)->pluck('login_time');

          //dd($test);
          if(is_null($test))
          {
            return Redirect::route('show_login',[request()->input('site_id'),$userid]);
          }
          else
          {
            $test1=DB::table('user_attendance')->where('userid',$userid)->where('created_date',$currentdate)->where('site_id',request()->input('site_id'))->where('flag_login',1)->pluck('logout_time');
            if(is_null($test1))
            {
              return Redirect::route('show_logout',[request()->input('site_id'),$userid]);
            }
            else
            {
              /*return Redirect::route('user.show',['id'=>request()->input('site_id')])
        ...
        {
          return Redirect::route('force_logout',[$userid, request()->input('site_id')]);
  ...
  public function show_login($site_id,$id)
  {
     ...
     $data=User::find($id);
     $site=[''=>'Please Select']+DB::table('site')->pluck('site_name','id');
  ...
  public function force_logout($id,$site_id)
  {
     ...
     $check=DB::table('user_attendance')->where('userid',$id)
        ...
        // dd($data);
    // $site=[''=>'Select Site']+DB::table('site')->pluck('site_name','id');
 ...
 public function post_forcelogout($id)
  {
    $input=request()->all();
    // dd($input);
    Arr::forget($input,['_token','site_id']);
    ...
    if($Validator->passes())
    {
      ...
      $logout_siteid=request()->input('logout_siteid');

      $logout_time=request()->input('date')." ".request()->input('time');
         // $logout_time="0000-00-00 00:00:00";
       
        $login_type=request()->input('login_type');
        $cdate=request()->input('date-end');
        $currentdateandtime=date('Y-m-d H:i:s',strtotime(request()->input('date')));
        ...
        $user_id=request()->input('userid');
         ...
         if ($login_type=="1") {
          $login_type_text="roster";
           ...
           $logout_time=$logged_out_date .' '.request()->input('time');
             $json = request()->input('customer_signature');
  ...
  public function loginupdate()
  {
      ...
      //var_dump(request()->all());
        ...
        $currentdate=date('Y-m-d',strtotime(request()->input('date')));
  ...
  public function logoutupdate()
  {
        ...
        $currentdate=date('Y-m-d',strtotime(request()->input('date')));
        $currentdateandtime=date('Y-m-d H:i:s',strtotime(request()->input('date')));
        $current_time=date('H:i:s',strtotime(request()->input('date')));
        ...
        $imgid=TimeSheet::where('userid',request()->input('userid'))->where('created_date',$currentdate)
        ->where('site_id',request()->input('site_id'))->where('flag_login',1)
        ->pluck('id');
  ...
  {
    $validator=Validator::make(request()->all(),['site_id'=>'required']);
    if($validator->passes())
    {
      return Redirect::route('user.show',['id'=>request()->input('site_id')]);
    }
