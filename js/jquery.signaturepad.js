...
function SignaturePad (selector, options) {
  ...
  function onFormError (errors, context, settings) {
    ...
    $("#name,#site,#date,#time").next("span").remove();
    ...
    {
      $("#date").after('<span class=error>There is no time entered, please try again.</span>');
    }
    if (errors.siteInvalid) 
    {
      $("#site").after('<span class=error>This field is required</span>');
    }
     if (errors.timeInvalid) 
    {
    ...
    {
       $("#sign-error").html('A signature is required to submit the timesheet. Please sign your name and try again');
    }
    else
    {
        $("#sign-error").html('');
  ...
  function validateForm () {
    var valid = true
      , errors = {drawInvalid: false, nameInvalid: false,dateInvalid:false,siteInvalid:false,timeInvalid:false}
    ...
    if ($('#date').val() === '') {
     ...
     if ($('#site').val() === '') {
      errors.siteInvalid = true
      valid = false
    }

     if ($('#time').val() === '') {
      errors.timeInvalid = true
      valid = false
    }
    ...
    if (element.getContext) {
      ...
      if (!settings.displayOnly) {
        ...
        if (settings.validateFields) {
          if ($(selector).is('form')) {
            $(selector).bind('submit.signaturepad', function () { if(validateForm()){$('#showtime,.btn,.btn-primary').prop('disabled',true); $("a").removeAttr('href');$('#showtime').val('Submitting');} return validateForm() })
          } else {
            $(selector).parents('form').bind('submit.signaturepad', function () { if(validateForm()){$('#showtime,.btn,.btn-primary').prop('disabled',true); $("a").removeAttr('href');$('#showtime').val('Submitting');} return validateForm() })
          }
        }
